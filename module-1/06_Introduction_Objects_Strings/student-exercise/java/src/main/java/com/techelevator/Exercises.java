...
public class Exercises {
	...
	public String extraEnd(String str) {
		String threeCopies = str.substring(str.length() - 2);
		return threeCopies + threeCopies + threeCopies;
	}
	...
	public String middleThree(String str) {
		...
		if (lengthOfStr == 3) {
			...
			return str.substring(lengthOfStr / 2 - 1, lengthOfStr / 2 + 2);
		}
	...
	public String frontTimes(String str, int n) {
		int frontThree = 3;
		if (frontThree > str.length()) {
			frontThree = str.length();
		}
		String front = str.substring(0, frontThree);
		String duplicate = "";
		int i = 0;
		while (i < n) {
			duplicate = duplicate + front;
			i++;
		}
		return duplicate;
	}
	...
	public String stringSplosion(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			splosionString = splosionString + str.substring(0, i + 1);
	...
	public String stringX(String str) {
		...
		char xStart = str.charAt(0);
		char xEnd = str.charAt(str.length() - 1);
		str = str.substring(1, str.length() - 1).replace("x", "");
		return xStart + str + xEnd;
	}
	...
	public String altPairs(String str) {
		...
		int i = 0;
		while (i < str.length()) {
			newPairs = newPairs + str.charAt(i);
			i++;
			if (i < str.length()) {
				newPairs = newPairs + str.charAt(i);
				i = i + 3;
	...
	/*
	 * String output = ""; for (int i = 0; i < str.length(); i += 4) { String
	 * letters = str.substring(i, i + 2); output = output + letters; if (i + 1 <
	 * str.length()) { output = output + str.substring(i + 1, i + 2); }
	 * 
	 * } return output;
	 */
	 ...
	 * all the "yak" are removed. The "yak" strings will not overlap.
