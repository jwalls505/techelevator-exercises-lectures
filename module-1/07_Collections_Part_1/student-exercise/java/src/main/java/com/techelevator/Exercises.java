...
public class Exercises {
	...
	public String[] list2Array(List<String> stringList) {
		String[] namesArray = new String[stringList.size()];
		stringList.toArray(namesArray);
		return namesArray;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (int i = 0; i < intArray.length; i++) {
			...
			double dividedInt = (double) result / 2;
			newArray.add(dividedInt);
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		for (int i = 0; i < integerList.size(); i++) {
			if (integerList.get(i) == intToFind && integerList.get(i + 1) == intToFind
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		Queue<Integer> firstRange = new LinkedList<Integer>();
		Queue<Integer> secondRange = new LinkedList<Integer>();
		Queue<Integer> thirdRange = new LinkedList<Integer>();
		List<Integer> finalList = new ArrayList<Integer>();

		for (Integer seatValue : seatNumberList) {
			if (seatValue >= 1 && seatValue <= 10) {
				firstRange.offer(seatValue);
			} else if (seatValue >= 11 && seatValue <= 20) {
				secondRange.offer(seatValue);
			} else if (seatValue >= 21 && seatValue <= 30) {
				thirdRange.offer(seatValue);

			} else {
				continue;
			}
		}
		finalList.addAll(firstRange);
		finalList.addAll(secondRange);
		finalList.addAll(thirdRange);
		return finalList;
	}
