...
-- We can see that the * pulls back everything from both tables. We just want everything from payment and then the first and last name of the customer:
        ...
        FROM payment
        INNER JOIN customer ON payment.customer_id = customer_id
        WHERE payment_id = 16666;
-- But when did they return the rental? Where would that data come from? From the rental table, so let’s join that.
        SELECT rental.return_date, payment.*, customer.first_name, customer.last_name
        FROM rental
        INNER JOIN payment ON payment.rental_id = rental.rental_id
        INNER JOIN customer   ON payment.customer_id = rental.rental_id  
        WHERE payment_id = 16666;
...
-- What if we wanted to know who acted in that film?
        SELECT film.title, actor.first_name, actor.last_name, customer.first_name, customer.last_name
        FROM film
